...
/**
 * MSP RADAR — DAILY MARKET INTELLIGENCE report view.
 * Rendered at /tools/msp-radar for paid customers (admins included). Data comes from /api/msp-radar/* which enforces
 * the paid entitlement server-side; every figure is the persisted daily report — nothing is recomputed client-side.
 * Admins additionally receive an `ops` block (delivery status, run id) from the API and see it inline.
 */
...
type Payload = { sessionDate: string; reportVersion: number; status: string; healthStatus: HealthStatus; headline: string; generatedAt: string; report: DailyReport; nav: { previous: string | null; next: string | null }; ops?: Ops };
type ArchiveItem = { sessionDate: string; headline: string; healthStatus: HealthStatus; status: string; generatedAt: string; emailStatus?: EmailStatus };
...
export default function MspRadarReport({ onAccessDenied }: { onAccessDenied?: (status: 401 | 403) => void } = {}) {
  ...
  const load = useCallback(async (date?: string | null) => {
    ...
    try {
      const res = await fetch(`${API}/daily${date ? `?date=${date}` : ""}`, { credentials: "include", cache: "no-store" });
      const json = await res.json();
      if (res.status === 401 || res.status === 403) { onAccessDenied?.(res.status); setData(null); return; }
  ...
  const loadArchive = useCallback(async (limit: number) => {
    try {
      const res = await fetch(`${API}/archive?limit=${limit}`, { credentials: "include", cache: "no-store" });
      const json = await res.json();
        ...
        <>
          ...
          <AdminCard title="Data quality" actions={<button style={btn} onClick={() => setShowHealth((s) => !s)}>{showHealth ? "Collapse" : "Details"}</button>} className="mb-4">
            <div style={{ display: "flex", flexWrap: "wrap", gap: "0.4rem" }}>
              ...
              {ops && <StatusPill label={`AV ${r.dataHealth.alphaVantageCalls} · CG ${r.dataHealth.coingeckoCalls} · ${(r.dataHealth.runtimeMs / 60000).toFixed(1)} min${r.dataHealth.peakRssMb !== null ? ` · ${r.dataHealth.peakRssMb} MB` : ""}`} />}
            </div>
            {showHealth && (
