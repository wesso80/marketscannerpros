  ...
  return (
    <div className="rounded-xl border border-white/5 bg-white/2 p-4">
      ...
      <div className="overflow-hidden rounded-lg border border-white/5">
        <div className="hidden grid-cols-5 gap-0 bg-white/5 px-3 py-2 text-xs text-slate-300 sm:grid">
          <div>TF</div>
          <div>State</div>
          <div>Bias</div>
          <div>Strength</div>
        ...
        {rows.map((row) => (
          <div key={row.tfLabel} className="grid grid-cols-2 gap-0 px-3 py-2 text-[10px] text-slate-200 sm:grid-cols-5 sm:text-xs">
