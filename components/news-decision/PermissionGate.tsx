  ...
  return (
    <article className="rounded-2xl border border-white/10 bg-white/5 p-4 lg:col-span-2">
      <div className="mb-2 flex items-start justify-between gap-3">
        <div>
          <div className="text-xs text-white/60">News Condition Gate</div>
          <div className="mt-1 flex items-center gap-2">
            <span className="text-sm font-semibold text-white/90">CONDITION:</span>
            <span
          ...
          <p className="mt-2 text-sm text-white/70">
            {gate.permission === 'NO'
              ...
              : gate.permission === 'CONDITIONAL'
                ? 'Risk is elevated — tighter risk controls and large-cap focus may apply.'
                : 'Narratives are stable enough for normal conditions.'}
          </p>
        </div>
        <div className="text-right text-xs text-white/60">
          Status: <span className="font-semibold text-white/85">{gate.executionMode}</span>
        </div>
      </div>
      ...
      <div className="space-y-1 text-xs text-white/70">
        ...
        <div>• If Risk-Off + Expansion → conditions suggest reduced exposure and large-cap focus.</div>
        <div>• If narrative is dominant + sentiment improving → trend conditions present.</div>
      </div>
    </article>
  );
}
