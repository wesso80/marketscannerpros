...
export async function POST(req: NextRequest) {
  ...
  try {
    ...
    // Indicator math lives in lib/scanner/indicatorMath (shared with Golden Egg); these aliases keep the closure API unchanged.
    const ema = scannerMath.ema;
    const rsi = scannerMath.rsi;
    const macd = scannerMath.macd;
    const atr = scannerMath.atr;
    const adx = scannerMath.adx;
    const stochastic = scannerMath.stochastic;
