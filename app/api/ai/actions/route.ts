...
async function ensureJournalRiskColumns() {
  ...
  await q(`ALTER TABLE IF EXISTS journal_entries ADD COLUMN IF NOT EXISTS leverage DECIMAL(10,2)`);
  await q(`ALTER TABLE IF EXISTS journal_entries ADD COLUMN IF NOT EXISTS trail_rule VARCHAR(30) DEFAULT 'NONE'`);
  await q(`ALTER TABLE IF EXISTS journal_entries ADD COLUMN IF NOT EXISTS time_stop_minutes INTEGER DEFAULT 0`);
  await q(`ALTER TABLE IF EXISTS journal_entries ADD COLUMN IF NOT EXISTS take_profit_2 DECIMAL(18,8)`);
  await q(`ALTER TABLE IF EXISTS journal_entries ADD COLUMN IF NOT EXISTS execution_mode VARCHAR(10) DEFAULT 'DRY_RUN'`);
  await q(`ALTER TABLE IF EXISTS journal_entries ADD COLUMN IF NOT EXISTS max_loss_usd DECIMAL(18,2)`);
