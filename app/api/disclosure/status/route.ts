...
export async function GET() {
  ...
  if (!session?.workspaceId) {
    return NextResponse.json({ accepted: false, authenticated: false, version: DISCLOSURE_VERSION }, { headers: { 'Cache-Control': 'no-store' } });
  }
  ...
  try {
    ...
    return NextResponse.json({ accepted: rows.length > 0, authenticated: true, version: DISCLOSURE_VERSION }, { headers: { 'Cache-Control': 'no-store' } });
  } catch {
