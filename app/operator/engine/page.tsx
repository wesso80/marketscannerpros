...
import type {
  RadarOpportunity, Permission, Regime, Playbook,
} from '@/types/operator';
...
export default function OperatorEnginePage() {
  ...
  useEffect(() => {
    if (!tier) return;
    // Only admin users reach this page — operator layout enforces auth
    // Double-check by calling the radar endpoint
    fetch('/api/operator/engine/radar')
      .then(r => {
        setAuthorized(r.ok);
        setLoading(false);
      })
    ...
    <div style={{ background: '#0F172A', minHeight: '100vh', color: '#E2E8F0', padding: '24px' }}>
      ...
      {scanResult && (
        <div style={{ marginBottom: 24 }}>
          ...
          <div style={{ fontSize: 12, color: '#64748B', marginBottom: 12 }}>
            Scanned {scanResult.symbolsScanned} symbols at {new Date(scanResult.timestamp).toLocaleTimeString()}
                ...
                <div key={i} style={{
                  ...
                  {opp.reasonCodes.length > 0 && (
                    <div style={{ marginTop: 8, fontSize: 11, color: '#64748B' }}>
                      {opp.reasonCodes.slice(0, 3).join(' · ')}
                    </div>
                  )}
                </div>
              ))}
      ...
      <div style={{
        ...
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(180px, 1fr))', gap: 12 }}>
          {[
            'Market Data', 'Feature Engine', 'Regime Engine', 'Doctrine Engine',
            'Playbook Engine', 'Scoring Engine', 'Governance Engine', 'Execution Engine',
            'Review Engine', 'Learning Engine',
          ].map(name => (
            <div key={name} style={{
              ...
              <div style={{ fontSize: 13, color: '#10B981', fontWeight: 600 }}>Ready</div>
            </div>
          ))}
        </div>
      </div>
