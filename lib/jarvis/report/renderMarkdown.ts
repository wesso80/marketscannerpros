...
export function renderReportMarkdown(r: DailyReport): string {
  ...
  o.push(`# ${warn ? '⚠ JARVIS DATA HEALTH WARNING' : 'JARVIS DAILY MARKET INTELLIGENCE'} — ${fmtDate(r.sessionDate)}`, '', `US session ${r.sessionDate} · run ${r.run.runKey} generated ${r.run.generatedAt} · report v${r.version} built ${r.generatedAt}`, '', `**${r.headline}**`, '', `_${r.disclaimer}_`, '');
  if (warn) {
  ...
  o.push('', '## E. LIFECYCLE CHANGES (jarvis_watchlist)', '', `Counts: ${Object.entries(lc.counts).filter(([, v]) => v > 0).map(([k, v]) => `${k} ${v}`).join(' · ') || 'none'}`, '', '**Highlights**', ...list(lc.highlights.map((t) => `${t.symbol}: ${t.from ?? 'new'} → **${t.to}** — ${t.note}`)), '', `_${lc.transitions.length} transitions recorded for ${r.sessionDate}._`);
