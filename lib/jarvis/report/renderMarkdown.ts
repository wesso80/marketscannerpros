...
import { classifyCaveat } from './types';
...
export function renderReportMarkdown(r: DailyReport): string {
  ...
  o.push('## C. TOP RESEARCH CANDIDATES', '', '_Largest mover ≠ best candidate: ranking is research quality (participation, structure, RS, theme, catalyst), not % change._', '', '| # | Symbol | Class | Setup | Score | Extension | 1d | 5d | Lifecycle | Why surfaced | Caveat / would reduce interest | Catalyst |', '|---|---|---|---|---|---|---|---|---|---|---|---|');
  for (const c of r.candidates) { const cv = classifyCaveat(c.caveat); o.push(`| ${c.rank} | **${c.symbol}** | ${c.assetClass} | ${c.setupType} | ${c.score} | ${c.extension} | ${c.ret1 > 0 ? '+' : ''}${c.ret1.toFixed(1)}% | ${c.ret5 === null ? 'n/a' : (c.ret5 > 0 ? '+' : '') + c.ret5.toFixed(1) + '%'} | ${c.lifecycle ?? '—'} | ${c.whySurfaced.slice(0, 140)} | ${cv.kind === 'observed' ? cv.text.slice(0, 110) : `_would reduce interest:_ ${cv.text.slice(0, 90)}`} | ${c.catalyst.slice(0, 90)} |`); }
