...
export interface EarningsAnalyzerNote {
  ...
  /** Options implied move — flagged as missing per AV limitation. */
  optionsImpliedMove: {
    available: false;             // hard false on free AV tier
    note: string;                 // explain why missing
...
- Options implied move is NEVER in the packet (AV has no options chain). Set optionsImpliedMove.available = false and use the historical avg/median earnings-day move from REACTION_STATS as the proxy. Be explicit that this is a backward-looking proxy, not a forward IV signal.
...
OUTPUT: Return ONE strict JSON object matching this TypeScript interface exactly:
{
  ...
  "optionsImpliedMove": { "available": false, "note": string, "historicalProxyAvgPct": number|null, "historicalProxyMedianPct": number|null },
...
export function validateEarningsNote(
  ...
  // Force-correct: optionsImpliedMove.available must be false (hard rule).
