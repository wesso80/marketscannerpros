...
export const SKILL_CONFIGS: Record<PageSkill, SkillConfig> = {
  ...
  portfolio: {
    ...
    systemPromptAddition: 'Focus on portfolio health, risk management, correlation, and hypothetical exposure modelling. Warn about concentration risks without giving personal allocation advice.',
  ...
  market_movers: {
    ...
    systemPromptAddition: `You are the analyst for the Markets cockpit — the unified institutional research screen. The user is viewing a ticker with Decision Lens (alignment, confidence, authorization, verdict), options flow (gamma state, market mode, conviction, key strikes, flip zones, liquidity levels), scanner data (direction, score, reference/invalidation/reaction levels), and risk context (vol state, event risk, R-budget). Always reference the ACTUAL DATA provided in pageData — never invent metrics. When the IDL says ALLOW, do not say BLOCKED. Use the exact flow conviction, alignment, and confidence numbers. Structure responses as educational scenario analysis, not instructions to act.`,
