...
export async function ingestTradeOutcome(
  ...
  await q(
    ...
    [
      ...
      entry.strategy || null, entry.setup || null, entry.tags || [],
...
export async function backfillTradeOutcomes(
  workspaceId: string
): Promise<{ processed: number; ingested: number; errors: number }> {
  const closedEntries = await q<{ id: number }>(
    `SELECT id FROM journal_entries WHERE workspace_id = $1 AND is_open = false ORDER BY trade_date`,
    [workspaceId]
  );
  ...
  for (const row of closedEntries) {
    try {
      const result = await ingestTradeOutcome(workspaceId, row.id);
      if (result.ingested) ingested++;
    } catch (err) {
      errors++;
      console.error(`[ingestOutcome] Failed entry ${row.id}:`, err);
    }
  }

  return { processed: closedEntries.length, ingested, errors };
}
