...
function generateInsights(slices: EdgeSlice[]): EdgeInsight[] {
  ...
  if (worst && worst.expectancy < 0) {
    insights.push({
      ...
      body: `${(worst.winRate * 100).toFixed(0)}% win rate with ${worst.avgR.toFixed(2)}R avg across ${worst.sampleSize} trades. Consider reducing size or skipping.`,
  ...
  if (highLoss) {
    insights.push({
      ...
      body: `Only ${(highLoss.winRate * 100).toFixed(0)}% wins with ${highLoss.avgR.toFixed(2)}R avg. Consider reviewing or avoiding.`,
