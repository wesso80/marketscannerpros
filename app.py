...
if not ios_issue_detected and not st.session_state.eq_results.empty:
    ...
    display_eq = st.session_state.eq_results.head(topk).copy()
    
    # Add AI Analyst link column
    def generate_ai_link(row):
        import urllib.parse
        ...
        dir_param = 'long' if direction.lower() == 'bullish' else 'short' if direction.lower() == 'bearish' else 'neutral'
        ...
        base = "https://marketscannerpros.app/tools/ai-analyst"
        return f"{base}?{urllib.parse.urlencode(params)}"
    
    display_eq['AI Analysis'] = display_eq.apply(lambda row: generate_ai_link(row), axis=1)
    ...
    if 'direction' in display_eq.columns:
        ...
        st.dataframe(
            ...
            column_config={
                "AI Analysis": st.column_config.LinkColumn(
                    "🤖 AI Analysis",
                    help="Get AI explanation for this signal",
    ...
    # Add "Explain in AI Analyst" buttons for top results
    st.markdown("#### 🤖 AI Analysis")
    cols = st.columns(min(3, len(display_eq)))
    for idx, (i, row) in enumerate(display_eq.head(3).iterrows()):
        with cols[idx]:
            symbol = row.get('symbol', 'N/A')
            price = row.get('close', 0)
            timeframe = st.session_state.get('selected_timeframe', '1h')
            ...
            # Convert direction to long/short for URL
            dir_param = 'long' if direction.lower() == 'bullish' else 'short' if direction.lower() == 'bearish' else 'neutral'
            
            ai_url = f"https://marketscannerpros.app/tools/ai-analyst?from=scanner&asset=stock&symbol={symbol}&tf={timeframe}&price={price:.2f}&dir={dir_param}&score={score/100:.2f}"
            st.link_button(
                f"💡 Explain {symbol}",
                ai_url,
                help=f"Get AI analysis for {symbol} signal",
                use_container_width=True
            )
...
if not ios_issue_detected and not st.session_state.cx_results.empty:
    ...
    # Limit display to top K
    display_cx = st.session_state.cx_results.head(topk).copy()
    
    # Add AI Analyst link column for crypto
    def generate_ai_link_crypto(row):
        import urllib.parse
        symbol = row.get('symbol', 'N/A')
        price = row.get('close', 0)
        timeframe = st.session_state.get('selected_timeframe', '1h')
    ...
    display_cx['AI Analysis'] = display_cx.apply(lambda row: generate_ai_link_crypto(row), axis=1)
    ...
    # Add "Explain in AI Analyst" buttons for top crypto results
    st.markdown("#### 🤖 AI Analysis")
    cols = st.columns(min(3, len(display_cx)))
    for idx, (i, row) in enumerate(display_cx.head(3).iterrows()):
        with cols[idx]:
            symbol = row.get('symbol', 'N/A')
            price = row.get('close', 0)
            timeframe = st.session_state.get('selected_timeframe', '1h')
            ...
            ai_url = f"https://marketscannerpros.app/tools/ai-analyst?from=scanner&asset=crypto&symbol={symbol}&tf={timeframe}&price={price:.4f}&dir={dir_param}&score={score/100:.2f}"
            st.link_button(
                f"💡 Explain {symbol}",
                ai_url,
                help=f"Get AI analysis for {symbol} signal",
