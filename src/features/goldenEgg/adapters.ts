  ...
  return {
    ...
    layer2: {
      ...
      scenario: {
        ...
        referenceLevel: { type: 'confirmation', price: 1143.0 },
        invalidationLevel: { price: 1116.5, logic: 'Below invalidation structure + failed reclaim' },
        reactionZones: [
          { price: 1168.0, rMultiple: 1.0, note: 'First reaction zone' },
          { price: 1196.0, rMultiple: 2.0, note: 'Primary reaction zone' },
          { price: 1224.0, rMultiple: 3.0, note: 'Extension reaction zone' },
        ],
        hypotheticalRr: { expectedR: 2.1, minR: 1.6 },
        hypotheticalRisk: { riskPct: 0.75, riskUsd: 750, sizeUnits: 28 },
      },
    },
    layer3: {
      structure: {
      ...
      narrative: {
        enabled: true,
        summary: 'Constructive trend context with acceptable scenario definition, but confirmation evidence is still required.',
        bullets: ['Hypothetical risk/reward improves only above the reference condition.', 'Flow confirmation remains the missing piece.'],
