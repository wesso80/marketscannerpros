...
describe('layout and flow audit regressions', () => {
  ...
  it('makes Terminal the handoff from Golden Egg to Backtest', () => {
    ...
    expect(terminalPage).toContain('<TerminalSubviewMetric label="Next Check" value={adjacentTab}');
    expect(terminalPage).toContain('Back to Calendar');
    expect(terminalPage).toContain('Open {adjacentTab}');
  ...
  it('keeps Dashboard as a morning research start page with compact secondary lenses', () => {
    ...
    // TR-11: real n/10 counter, not a hard-coded 0/10.
    expect(edgeInsightCards).toContain('{lock.progressLabel} — unlocks after {lock.unlockAt} journal closes');
