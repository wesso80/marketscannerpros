...
Evidence baseline:
- Scanner upgrade to lift Ranked, Pro, and Analysis modes to the Dashboard A-grade command standard: introduced an `aria-label="Scanner command header"` gradient section that folds the regime/risk/permission strip into a single inline eyebrow chip (with regime weights moved to a tooltip on the chip) and exposes four mode-aware metric tiles (`Mode`, `Queue`/`Symbol`, `Data Health`, `Next Check`); replaced the Ranked/Pro/Analysis flow rail's horizontal-scroll Terminal-style buttons with a fixed three-column grid; trimmed the boilerplate `; rank is reduced when evidence is missing, stale, or liquidity is thin.` suffix from ranked reason summaries at render-time; restyled the Pro `Run Educational Scan` button from a full-width emerald gradient to the standard amber CTA tone; replaced strategy-template emoji icons with text codes (`MOM`/`BRK`/`REV`/`SQZ`/`RS`/`ALN`); rewrote the Analysis `Bias: BULLISH` shouting label to `Bias: Bullish/Bearish/Neutral`; tinted the Analysis `Open Historical Test` CTA amber to match other validation paths; collapsed the Analysis `Key Levels (Educational)` block to a single line when all four levels are unavailable; extended the global favorite-button exclusion to also cover `/tools/scanner` so the standalone star chrome no longer competes with the new command header. Regression guards extended in `test/layoutFlowAudit.test.ts`.
- Dashboard polish to make it a true A-grade public-site standard: collapsed duplicate `MARKET REGIME` chrome into a single inline eyebrow chip beside the command title (regime, risk, sizing in one line); replaced the static `Headlines` metric tile with an adaptive `Next Check` tile that switches copy between `Validate <topQueueSymbol>` and `Run Scanner first` based on cached queue state; the `Validate Symbol` CTA and Continue Workflow step 2 now deep-link to `/tools/golden-egg?symbol=<topQueueSymbol>` when the queue is populated; loading-feed tone moved from sky blue to neutral slate so loading no longer reads as healthy; the empty Edge Profile card collapsed to a one-line compact strip. Regression guards extended in `test/layoutFlowAudit.test.ts`.
- Full-page Terminal scroll QA against Dashboard quality: Terminal now uses a Dashboard-style metric-backed command header with symbol, active lens, data state, and next-check tiles. The mechanics rail wraps responsively instead of showing a horizontal scrollbar, making Terminal visually closer to the current public-site standard while keeping the close-calendar table flow intact.
- Screenshot-driven Terminal QA after the route-local chrome cleanup: `/tools/terminal` no longer receives the global collapsible compliance banner or standalone favorite-star row, so the first viewport gets from regime/context into the Terminal command hero, symbol loader, and mechanics rail faster while preserving the route-owned compact risk disclaimer. Layout regression coverage was added in `test/layoutFlowAudit.test.ts`.
- Follow-on residual tools-page scan after `cb8bd084`: the targeted `app/tools/**/page.tsx` route-local chrome patterns now return clean for `<SectionHeader`, `ComplianceDisclaimer collapsible`, emoji header icons, and the current pictogram set. Final fixes covered Signal Accuracy threshold markers, Portfolio close/delete/standalone compliance chrome, and Alerts header identity. Tests in `test/layoutFlowAudit.test.ts` were updated before validation.
- Continuation after `e730edad`: the next specialist-route chrome batch replaced remaining Options Confluence validation/scenario pictograms, Equity Explorer quick-signal/refresh/error glyphs, Crypto console tab/error/close icons, Crypto Explorer error glyphs, and Crypto Dashboard derivatives signal emoji with compact `OK`/`NO`/`WARN`/`BULL`/`BEAR`/short-code labels. `test/layoutFlowAudit.test.ts` now guards these route-local surfaces. Continue with fresh scans for remaining page-local indicators before returning to deeper product/math audit work.
- Continuation after pulling `d6b4a54f`: route-local legacy chrome cleanup advanced through Backtest lower panels, Scanner Backtest replay alerts, Settings risk-guard warnings, Referrals contest/leaderboard copy, Gainers/Losers route chrome, and Market Movers alignment status labels. Added `test/layoutFlowAudit.test.ts` guards for these surfaces. Next priority remains lower-traffic route-local indicators found by scanning `app/tools/**/page.tsx`, especially Options Confluence and remaining Explorer/Crypto specialist pages; keep changes UI-only unless a touched surface exposes live/demo ambiguity.
- Laptop handoff after the current continuation: pull `main` before continuing if this batch has been pushed. Recent pushed UI consistency commits are `74baf71b` shared compact tool headers, `e696f5e9` Workspace/Journal/Scanner/Backtest local shell normalization, `218a5323` Research/Explorer compact hub shells, `3162e970` audit handoff notes, `5061d394` Market Movers/News/Commodities/Referrals chrome cleanup, and `1d9f673a` Company Overview/Heatmap/Economic Calendar/Macro chrome cleanup. This continuation also removed residual Market Movers and News warning/calendar pictograms found by the post-push scan; after push, continue public/app tool consistency before returning to the private admin rebuild.
- Exact next route batch: remove remaining route-local legacy chrome found by scanning `app/tools/**/page.tsx` for `<SectionHeader`, `ComplianceDisclaimer collapsible`, and `icon="...emoji..."`. Priority order after this batch: remaining Backtest lower-panel emoji/status labels and lower-traffic specialist pages. Convert visible emoji or large pictogram badges into compact text codes, switch dense tool disclaimers to `ComplianceDisclaimer compact` where appropriate, and avoid changing API contracts or data behavior unless the touched UI exposes fake/live ambiguity.
- Verification baseline for next agent: `npm exec vitest run test/layoutFlowAudit.test.ts test/public-compliance-copy.test.ts --reporter=dot` passed `22/22`; full `npm run build` passed with `300/300` pages; `git diff --check` was clean. Keep adding/adjusting `test/layoutFlowAudit.test.ts` string guards for each route batch and note completed work in both audit docs before committing.
...
| `/tools/options`, `/tools/options-terminal`, `/tools/options-confluence` | pro_trader | Options context, confluence, terminal | 8 | 6 | 6 | 6 | MERGED into `/tools/terminal` tabs |
...
| `/tools/options-terminal` | Good terminal direction and Pro Trader positioning. | Several tabs and features overlap with options/confluence/flow pages. | Completed: `/tools/options`, `/tools/options-terminal` now server-redirect to `/tools/terminal?tab=options-terminal`; `/tools/options-flow` has noindex layout canonicalizing to Terminal; `toolCatalog.ts` deduplicated to 3 options entries (OT/OC/OF) all pointing to Terminal tabs. |
...
| `/tools/options-terminal` | B- | Overlap between options tools makes navigation heavy. | Completed: all options routes canonicalize to `/tools/terminal` tabs. | No |
