...
Evidence baseline:
- Laptop handoff after commit `218a5323` (`Compact research explorer shells`): pull `main` before continuing. Recent pushed UI consistency commits are `74baf71b` shared compact tool headers, `e696f5e9` Workspace/Journal/Scanner/Backtest local shell normalization, and `218a5323` Research/Explorer compact hub shells. The next elite-design continuation is not admin yet; keep working through public/app tool consistency so the platform feels like one terminal-grade product before returning to the private admin rebuild.
- Exact next route batch: remove remaining route-local legacy chrome found by scanning `app/tools/**/page.tsx` for `<SectionHeader`, `ComplianceDisclaimer collapsible`, and `icon="...emoji..."`. Priority order: Market Movers, News, Commodities, Referrals, then Company Overview, Heatmap, Economic Calendar, Macro, and remaining Backtest lower-panel emoji/status labels. Convert visible emoji or large pictogram badges into compact text codes, switch dense tool disclaimers to `ComplianceDisclaimer compact` where appropriate, and avoid changing API contracts or data behavior unless the touched UI exposes fake/live ambiguity.
