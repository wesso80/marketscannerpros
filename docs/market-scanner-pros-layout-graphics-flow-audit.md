...
- Laptop handoff after commit `218a5323` (`Compact research explorer shells`): branch `main` was clean and synced with `origin/main` after the push. The continuation target is the remaining route-local legacy chrome in `app/tools/**/page.tsx`: `ToolsPageHeader` icon props still using emoji/text pictograms, `ComplianceDisclaimer collapsible` instances that should become `compact`, and page-local emoji/state badges on lower-traffic specialist pages. Start with high-visibility canonical/embedded surfaces from the latest scan: `app/tools/market-movers/page.tsx`, `app/tools/news/page.tsx`, `app/tools/commodities/page.tsx`, `app/tools/referrals/page.tsx`, then `app/tools/company-overview/page.tsx`, `app/tools/heatmap/page.tsx`, `app/tools/economic-calendar/page.tsx`, `app/tools/macro/page.tsx`, and remaining Backtest lower-panel emoji labels. Keep changes route-scoped and preserve embedded hub behavior for Research/Explorer/Workspace.
