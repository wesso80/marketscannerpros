...
- Laptop handoff after this continuation: latest pushed route batch before these edits is `5061d394` (`Compact market context tool chrome`). This local continuation has also cleaned `app/tools/company-overview/page.tsx`, `app/tools/heatmap/page.tsx`, `app/tools/economic-calendar/page.tsx`, and `app/tools/macro/page.tsx` by replacing page-local icon props, visible section pictograms, and warning glyphs with compact text-code labels. Next route-local legacy chrome targets are remaining Backtest lower-panel emoji/status labels and lower-traffic specialist pages found by a fresh scan of `app/tools/**/page.tsx`. Keep changes route-scoped and preserve embedded hub behavior for Research/Explorer/Workspace/Golden Egg.
