...
- Laptop handoff after this in-progress batch: previous pushed handoff was commit `3162e970` (`Add audit handoff notes`). This local continuation has now cleaned the first high-visibility route batch: `app/tools/market-movers/page.tsx`, `app/tools/news/page.tsx`, `app/tools/commodities/page.tsx`, and `app/tools/referrals/page.tsx` have compact disclaimers and text-code header identities, with route-local news/commodity/mover pictograms replaced by compact text labels. Next route-local legacy chrome targets are `app/tools/company-overview/page.tsx`, `app/tools/heatmap/page.tsx`, `app/tools/economic-calendar/page.tsx`, `app/tools/macro/page.tsx`, and remaining Backtest lower-panel emoji/status labels. Keep changes route-scoped and preserve embedded hub behavior for Research/Explorer/Workspace.
